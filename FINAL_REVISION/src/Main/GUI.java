...
public class GUI {
	...
	private int waveBarX = 0;
	private int waveBarY = 566;

	private Game game; //singleton reference to the implementing game
	public GUI(Game g) {
		game = g;
		setFont(new SystemFont(new Font("Arial", Font.BOLD|Font.PLAIN, 28), Color.BLUE));
	}
	...
	public void render(Graphics2D g) {
		...
		getFont().drawString(g, "Lives: "+game.getEnviro().getLives(), 650, waveBarY);
		getFont().drawString(g, "Points: "+game.getMoney(), waveBarX, waveBarY);
		getFont().drawString(g,"Diff: " + customFormat("#.##", game.getDifficulty()), waveBarX+200, waveBarY);
		getFont().drawString(g, ("Time: " + (game.getSeconds()/60)+":"+customFormat("00",(game.getSeconds()-((game.getSeconds()/60)*60)))), waveBarX+400, waveBarY);
	}
