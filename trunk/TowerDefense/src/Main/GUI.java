...
public class GUI {
			...
			try {
				if(game.getMoney()>100) {
					if(((Tile) game.getSpriteUnderMouse(game.getMouseX(), game.getMouseY())).canBuild() && ((Tile) game.getSpriteUnderMouse(game.getMouseX(), game.getMouseY())).sprite==null) {
						...
						game.enviro.addTower((Tower)selectedTower);
					}
	...
	private Tower checkClick(int x, int y) {
		if(x>700 && y>0 && y<75) {
			return (Tower)(new WaterTower(game.waterTowerImage, game.waterProjImage, 1, game.enviro));
		}
		if(x>700 && y>100 && y<173) {
			return (Tower)(new EarthTower(game.earthTowerImage, game.earthProjImage, 1, game.enviro));
		}
		if(x>700 && y>200 && y<273) {
			return (Tower)(new FireTower(game.fireTowerImage, game.fireProjImage, 1, game.enviro));
		}
		if(x>700 && y>300 && y<373) {
			return (Tower)(new AirTower(game.airTowerImage, game.airProjImage, 1, game.enviro));
		}
		else return null;
	}
